...
public class GameObject
{
	...
	*/
	...
	/** rotates this GameObject around the local Up axis by the specified amount */
    public void yaw(float amount)
    {   
		Matrix4f oldRotation = new Matrix4f(this.getLocalRotation());
		Vector3f dolphinUp = this.getWorldUpVector(); // Note the change to Up vector
    	Matrix4f addedRotation = (new Matrix4f()).rotation(amount, dolphinUp);
		Matrix4f newRotation = new Matrix4f();
		addedRotation.mul(oldRotation, newRotation);
		this.setLocalRotation(newRotation);
    }

	/** rotates this GameObject around the WORLD Up axis (0,1,0) */
	public void globalYaw(float amount)
	{
    	Matrix4f oldRotation = new Matrix4f(this.getLocalRotation());
   		// Use the absolute world up vector (0, 1, 0)
    	Vector3f worldUp = new Vector3f(0f, 1f, 0f); 
    	Matrix4f addedRotation = (new Matrix4f()).rotation(amount, worldUp);
    	Matrix4f newRotation = new Matrix4f();
    	// POST-MULTIPLICATION (oldRotation * addedRotation) 
    	// This applies the rotation relative to the world axes.
    	oldRotation.mul(addedRotation, newRotation);
    	this.setLocalRotation(newRotation);
	}

    /** rotates this GameObject around the local Right axis by the specified amount */
    public void pitch(float amount)
    {   
		Matrix4f oldRotation = new Matrix4f(this.getLocalRotation());
		Vector3f dolphinRight = this.getWorldRightVector();
		Matrix4f addedRotation = (new Matrix4f()).rotation(amount, dolphinRight);
		Matrix4f newRotation = new Matrix4f();
    	addedRotation.mul(oldRotation, newRotation);
		this.setLocalRotation(newRotation);
    }
